...
To mock existing class properties, `Overwrites.Mock(() => PropertyName)` routes the property getter to an `IAuto{ClassName}Dependencies` member.

### Deterministic Naming
Generated interface members use a deterministic naming convention derived from the fully qualified name (or access path) to ensure consistency and avoid collisions.
...
**Generated Interface Resolution:**
- `DateTime.Now` → `DateTime_Now` (Deterministic)
- `Now` → `Now` (Property name)
...
After (Generated):
```csharp
public class UserViewModel_G {
    public DateTime Now => _dependencies.Now; 
    // DateTime.Now call within constructor/methods replaced by _dependencies.DateTime_Now
}
```
