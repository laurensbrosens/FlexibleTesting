### 1) Base class for instruction files (`TestabilityInstructions.cs`)
...
### 2) Source generator (`LegacyTestabilityGenerator.cs`)
...
namespace LegacyTestability.Generator;
...
public sealed class LegacyTestabilityGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor RewriteMethodNotFound = new(
        id: "LTG004",
        title: "Rewrite method not found",
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var rules = instructionClasses.Select(static (cls, ct) => ExtractRules(cls, ct));

        // Flatten all extracted rules and generate
        var allRules = rules.SelectMany(static (arr, _) => arr);

        context.RegisterSourceOutput(allRules, static (spc, rule) => Generate(spc, rule));
    }

    private static INamedTypeSymbol? TryGetInstructionClass(GeneratorSyntaxContext ctx, CancellationToken ct)
    {
    ...
    private static ImmutableArray<TargetRule> ExtractRules(INamedTypeSymbol instructionClass, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested();
        ...
        foreach (var syntaxRef in configure.DeclaringSyntaxReferences)
        {
            ...
            var model = configure.DeclaringSyntaxReferences.Length > 0
                ? instructionClass.DeclaringSyntaxReferences.FirstOrDefault()?.SyntaxTree
                : null;

            // We need a semantic model from the tree where Configure lives.
            var semanticModel = mds.SyntaxTree is not null
                ? instructionClass.ContainingCompilation.GetSemanticModel(mds.SyntaxTree)
                : null;
    ...
    private static void Generate(SourceProductionContext spc, TargetRule rule)
    {
        ...
        var semanticModel = target.ContainingCompilation.GetSemanticModel(classDecl.SyntaxTree);
    ...
    private sealed class CloneRewriter : CSharpSyntaxRewriter
    {
        ...
        private static SyntaxTokenList Publicize(SyntaxTokenList modifiers)
        {
            ...
            {
                if (m.IsKind(SyntaxKind.PrivateKeyword) ||
...
## Example usage (in your **test project**) — instruction file
...
public sealed class MyLegacyRules : TestabilityInstructions
{
    public override void Configure()
    {
        For<ClassToTest>()
            ...
            .Publicize("Load", "Save", "ComputeInternal")
            .RewriteMethod("Now", "return new System.DateTime(2000, 1, 1);")
            .InjectMember(@"public string __TestHook() => ""ok"";");
    }
}
```

After build, you’ll get a generated type named:

- `ClassToTest_TestClass` (in the same namespace as `ClassToTest`)
- in a generated file hint like `ClassToTest_TestClass.g.cs`

And your unit tests can target `ClassToTest_TestClass` instead of `ClassToTest`.

If you want, tell me what your fluent syntax _must_ look like (e.g., using `Expression<TDelegate>` instead of strings), and I’ll adapt the PoC to parse and rewrite based on symbol-bound expressions rather than member-name strings.
